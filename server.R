...
shinyServer(function(input, output) {
  ...
  user_data <- reactive({
    if (input$example.data == TRUE) {
      user_data = read.csv("PCB-01-TRB-050.txt",
                           sep = "",
                           header = F)
      colnames(user_data) <- c("B", "M")
    } else
      ({
        inFile <- input$file1
        if (is.null(inFile))
    ...
    if (user_data[1, 1] == 0) {
      user_data[1, 1] = mean(c(user_data[1, 1], user_data[2, 1]))
    }
    user_data[, 3] <- log(user_data[, 1], 10) #V3
    user_data[, 4] <- smooth.spline(log(user_data[, 1], 10),
                                    user_data[, 2],
                                    spar = sf)$y #V4
    user_data[, 5] <-
      abs(predict(smooth.spline(
        log(user_data[, 1], 10),
        user_data[, 2],
        spar = sf
      ),
      deriv = 1)$y) #V5
    user_data[, 6] <- smooth.spline(user_data[, 1],
                                    user_data[, 2],
                                    spar = sf)$y #V6
    user_data[, 7] <- abs(predict(smooth.spline(user_data[, 1],
                                                user_data[, 2],
                                                spar = sf),
                                  deriv = 1)$y) #V7
    user_data[, 8] <-
      ...
      deriv = 1)$y) #V8
    user_data[, 9] <- abs(predict(smooth.spline(user_data[, 1],
                                                user_data[, 2],
                                                spar = 0),
                                  deriv = 1)$y) #V9
    user_data[, 10] <- smooth.spline(log(user_data[, 1], 10),
                                     user_data[, 8],
                                     spar = sf)$y #V10
    user_data[, 11] <- smooth.spline(user_data[, 1],
                                     user_data[, 9],
                                     spar = sf)$y #V11
    user_data
  })
  ...
  # observeEvent(user_data(),{vals$keeprows <- rep(TRUE, nrow(user_data()))})
  ## problem now is that the observer here needs to watch something
  ## that doesn't reset very often
  ...
  observeEvent(input$plot1_click, {
    res <- nearPoints(
      ...
      input$plot1_click,
      allRows = TRUE,
      xvar = if (input$scale == "linear") {
        if (input$header == F) {
          "B"
        }
        else
          (colnames(user_data())[1])
      } else
        ({
          "V3"
        })
      ...
      yvar = if (input$plot.type == "magnetization data") {
        if (input$header == F) {
          ...
          (colnames(user_data()[2]))
      } else
        ({
          if (input$plot.type == "coercivity spectra") {
            if (input$scale == "log") {
              "V8"
            }
            else
  ...
  observeEvent(input$exclude_reset, {
    vals$keeprows <- rep(TRUE, nrow(user_data()))
  })
  
  # plots on first page ----------------------------------------------------------
  
  output$plot1 <- renderPlot({
    if (is.null(user_data()) == FALSE)
    {
      user_data = user_data()
      inScale <- input$scale
      sf <- input$smooth.factor
      ...
      {
        dat_keep    <- user_data[vals$keeprows, , drop = FALSE]
        dat_exclude <- user_data[!vals$keeprows, , drop = FALSE]
      } else
        ({
          dat_keep = user_data
          dat_exclude = user_data
        })
      ...
      if (input$scale == "log") {
        if (input$plot.type == "magnetization data") {
          plot(
            dat_keep[, 3],
            dat_keep[, 2],
            xlim = c(min(dat_keep[, 3]), max(dat_keep[, 3])),
            ylim = c(0, max(dat_keep[, 2])),
            xlab = paste("log B (", input$B.units, ")"),
            ylab = paste("M (", input$M.units, ")"),
            col = "dark grey",
            pch = 19
          )
          lines(
          ...
          ({
            plot(
              ...
              type = "p",
              pch = 19,
              xlim = c(min(user_data[, 3]), max(dat_keep[, 3])),
              ylim = c(0, max(dat_keep[, 8])),
              xlab = paste("log B (", input$B.units, ")"),
              ylab = "dM/dlog(B)"
            )
            points(dat_exclude[, 3],
        ...
        ({
          if (input$plot.type == "magnetization data") {
            plot(
              dat_keep[, 1],
              dat_keep[, 2],
              xlim = c(min(dat_keep[, 1]), max(dat_keep[, 1])),
              ylim = c(0, max(dat_keep[, 2])),
              col = "dark grey",
              pch = 19,
              xlab = paste("B (", input$B.units, ")"),
              ylab = paste("M (", input$M.units, ")")
            )
            lines(
              dat_keep[, 1],
  ...
  output$ui.comp1 <- renderUI({
    conditionalPanel(
      ...
      ,
      if (input$save_1 > 0) {
        actionButton("reset1", "Reset to last save")
      },
      if (input$scale == "log") {
        sliderInput(
          "B1",
          "Mean Coercivity",
          min = 0,
          max = input$max.scale.log,
          value = if (is.null(input$reset1) == T) {
            if (input$save_1 == 0) {
              0
            }
            else
  ...
  output$ui.comp2 <- renderUI({
    conditionalPanel(
      ...
      ,
      if (input$save_2 > 0) {
        actionButton("reset2", "Reset to last save")
      },
      if (input$scale == "log") {
        sliderInput(
          "B2",
          ...
          min = 0,
          max = input$max.scale.log,
          value = if (is.null(input$reset2) == T) {
            if (input$save_2 == 0) {
              0
            }
            else
      ...
      if(input$scale=="log") {
        ...
        }), sliderInput("P2", "Relative Proportion:", min = 0, max = 1.0, value = if (is.null(input$reset2) ==
                                                                                T) {
        if (input$save_2 == 0) {
          0
        } else
  ...
  output$ui.comp3 <- renderUI({
    conditionalPanel(
      ...
      ,
      if (input$save_3 > 0) {
        actionButton("reset3", "Reset to last save")
      },
      if (input$scale == "log") {
        sliderInput(
          "B3",
          ...
          min = 0,
          max = input$max.scale.log,
          value = if (is.null(input$reset3) == T) {
            if (input$save_3 == 0) {
              0
            } else
              (save_comp_3()[1])} else
  ...
  output$ui.comp4 <- renderUI({
    conditionalPanel(
      ...
      ,
      if (input$save_4 > 0) {
        actionButton("reset4", "Reset to last save")
      },
      if (input$scale == "log") {
        sliderInput(
          "B4",
          ...
          min = 0,
          max = input$max.scale.log,
          value = if (is.null(input$reset4) == T) {
            if (input$save_4 == 0) {
              0
            } else
              (save_comp_4()[1])} else
  ...
  output$ui.comp5 <- renderUI({
    conditionalPanel(
      ...
      ,
      if (input$save_5 > 0) {
        actionButton("reset5", "Reset to last save")
      },
      if (input$scale == "log") {
        sliderInput(
          "B5",
          ...
          min = 0,
          max = input$max.scale.log,
          value = if (is.null(input$reset5) == T) {
            if (input$save_5 == 0) {
              0
            } else
              (save_comp_5()[1])} else
  ...
  output$ui.comp6 <- renderUI({
    conditionalPanel(
      ...
      ,
      if (input$save_6 > 0) {
        actionButton("reset6", "Reset to last save")
      },
      if (input$scale == "log") {
        sliderInput(
          "B6",
          ...
          min = 0,
          max = input$max.scale.log,
          value = if (is.null(input$reset6) == T) {
            if (input$save_6 == 0) {
              0
            } else
              (save_comp_6()[1])} else
  ...
  output$plot2 <- renderPlot({
    if (is.null(user_data()) == FALSE) {
      ...
      #### toggling points ########
      if (is.null(vals$keeprows) == FALSE) {
        user_data <- user_data[vals$keeprows, , drop = FALSE]
      }
      
      if (input$scale == "log") {
        M.sm = user_data[, 4]
        G = user_data[, 8]
        #G.sm = abs(predict(smooth.spline(log(user_data[,1],10),user_data[,2],spar=sf),seq(min(user_data[,3]),max(user_data[,3]),0.01),deriv=1)$y)
        B = user_data[, 3]
        B.d = seq(min(user_data[, 3]), max(user_data[, 3]), 0.01)
        if (input$smooth_magnetization == TRUE) {
          G.sm = approx(
            x = user_data[, 3],
            y = user_data[, 5],
        ...
        ({
          M.sm = user_data[, 6]
          G = user_data[, 9]
          #G.sm = abs(predict(smooth.spline(user_data[,1],user_data[,2],spar=sf),seq(min(user_data[,1]),max(user_data[,1]),1.0),deriv=1)$y)
          B = user_data[, 1]
          B.d = seq(min(user_data[, 1]), max(user_data[, 1]), 1.0)
          if (input$smooth_magnetization == TRUE) {
            G.sm = approx(
              x = user_data[, 1],
              y = user_data[, 7],
      ...
      if (n.comp == 1) {
        if (input$skew.option == FALSE) {
          C[, 1] = Pi[3] * (max(G.sm) * (dsnorm(B.d, Pi[1], Pi[2], S[1]) / max(dsnorm(
            B.d, Pi[1], Pi[2], S[1]
          ))))
        } else
          ({
            C[, 1] = Pi[3] * (max(G.sm) * (dnorm(B.d, Pi[1], Pi[2]) / max(dnorm(
              B.d, Pi[1], Pi[2]
            ))))
          })
        G.model = C[, 1]
      ...
      if (n.comp == 2) {
        if (input$skew.option == FALSE) {
          ...
          C[, 2] = Pi[6] * (max(G.sm) * (dsnorm(B.d, Pi[4], Pi[5], S[2]) /
                                           max(dsnorm(
                                             B.d, Pi[4], Pi[5], S[2]
                                           ))))
        } else
          ({
            ...
            C[, 2] = Pi[6] * (max(G.sm) * (dnorm(B.d, Pi[4], Pi[5]) / max(dnorm(
              B.d, Pi[4], Pi[5]
            ))))
          })
        G.model = C[, 1] + C[, 2]
      ...
      if (n.comp == 3) {
        if (input$skew.option == FALSE) {
          ...
          C[, 3] = Pi[9] * (max(G.sm) * (dsnorm(B.d, Pi[7], Pi[8], S[3]) /
                                           max(dsnorm(
                                             B.d, Pi[7], Pi[8], S[3]
                                           ))))
        } else
          ({
            C[, 1] = Pi[3] * (max(G.sm) * (dnorm(B.d, Pi[1], Pi[2]) / max(dsnorm(
              B.d, Pi[1], Pi[2]
            ))))
            C[, 2] = Pi[6] * (max(G.sm) * (dnorm(B.d, Pi[4], Pi[5]) / max(dsnorm(
              B.d, Pi[4], Pi[5]
            ))))
            C[, 3] = Pi[9] * (max(G.sm) * (dnorm(B.d, Pi[7], Pi[8]) / max(dsnorm(
              B.d, Pi[7], Pi[8]
            ))))
          })
        G.model = C[, 1] + C[, 2] + C[, 3]
      ...
      if (n.comp == 4) {
        if (input$skew.option == FALSE) {
          ...
          C[, 4] = Pi[12] * (max(G.sm) * (dsnorm(B.d, Pi[10], Pi[11], S[4]) /
                                            max(dsnorm(
                                              B.d, Pi[10], Pi[11], S[4]
                                            ))))
        } else
          ({
            ...
            C[, 3] = Pi[9] * (max(G.sm) * (dnorm(B.d, Pi[7], Pi[8]) / max(dnorm(
              B.d, Pi[7], Pi[8]
            ))))
            C[, 4] = Pi[12] * (max(G.sm) * (dnorm(B.d, Pi[10], Pi[11]) / max(dnorm(
              B.d, Pi[10], Pi[11]
            ))))
          })
      ...
      if (n.comp == 5) {
        if (input$skew.option == FALSE) {
          ...
          C[, 5] = Pi[15] * (max(G.sm) * (dsnorm(B.d, Pi[13], Pi[14], S[5]) /
                                            max(dsnorm(
                                              B.d, Pi[13], Pi[14], S[5]
                                            ))))
        } else
          ({
            ...
            C[, 5] = Pi[15] * (max(G.sm) * (dnorm(B.d, Pi[13], Pi[14]) / max(dnorm(
              B.d, Pi[13], Pi[14]
            ))))
          })
      ...
      if (n.comp == 6) {
        if (input$skew.option == FALSE) {
          ...
          C[, 6] = Pi[18] * (max(G.sm) * (dsnorm(B.d, Pi[16], Pi[17], S[6]) /
                                            max(dsnorm(
                                              B.d, Pi[16], Pi[17], S[6]
                                            ))))
        } else
          ({
            ...
            C[, 6] = Pi[18] * (max(G.sm) * (dnorm(B.d, Pi[16], Pi[17]) / max(dnorm(
              B.d, Pi[16], Pi[17]
            ))))
          })
      ...
      plot(
        ...
        xlab =
          if (input$scale == "log") {
            paste("log B (", input$B.units, ")")
          }
        else
          ({
            paste("B (", input$B.units, ")")
          })
        ,
  ...
  save_comp_1 <-
    reactive({
      # saving component as reactive object that only gets updated when input$save_1 action button clicked
      input$save_1
      isolate({
        c(input$B1, input$DP1, input$P1, input$S1)
      })
    })
  save_comp_2 <-
    reactive({
      ...
      input$save_2
      isolate({
        c(input$B2, input$DP2, input$P2, input$S2)
      })
    })
  save_comp_3 <-
    reactive({
      ...
      input$save_3
      isolate({
        c(input$B3, input$DP3, input$P3, input$S3)
      })
    })
  save_comp_4 <-
    reactive({
      ...
      input$save_4
      isolate({
        c(input$B4, input$DP4, input$P4, input$S4)
      })
    })
  save_comp_5 <-
    reactive({
      ...
      input$save_5
      isolate({
        c(input$B5, input$DP5, input$P5, input$S5)
      })
    })
  save_comp_6 <-
    reactive({
      ...
      input$save_6
      isolate({
        c(input$B6, input$DP6, input$P6, input$S6)
      })
    })
  ...
  output$plot3 <- renderPlot({
    if (is.null(user_data()) == FALSE) {
      ...
      user_data.a = auc(B.d,G.sm, from = min(B.d), to = max(B.d), type="spline")
      ...
      output$plot4<-renderPlot({
        ...
        observeEvent(input$calc.error,{  # start button begins
          ...
          if(input$scale == "log"){
            B.r = seq(min(user_data[,3]),max(user_data[,3]),by=0.01)
          }
          else({
            B.r = seq(min(user_data[,1]),max(user_data[,1]),by=1.0)
          })
          
          p.r = input$p.r
          
          for (i in 1:n){
            s[i] = list(user_data[sample(1:nrow(user_data),round(p.r*nrow(user_data)),replace=FALSE),]) #subset of data for spline fitting
            ...
            if(input$scale =="log"){
              if(input$smooth_magnetization == TRUE){
                ...
                if(input$smooth_gradient == TRUE){
                  ...
                  sp[i] = list(abs((predict(sp_temp[[i]],user_data[,3],deriv=1)$y))) #should be y-vals for the raw derivative, ie no smoothing
                }
              })
            }
            else({
              if(input$smooth_magnetization == TRUE){
                ...
                if(input$smooth_gradient == TRUE){
                  ...
                  sp[i] = list(abs((predict(sp_temp[[i]],user_data[,1],deriv=1)$y))) #should be y-vals for the raw derivative, ie no smoothing
                }
              })
            })
          }
          for (i in 1:n){
            if(input$scale == "log"){
              if(input$smooth_magnetization == TRUE){
                z[i] = list(approx(x = user_data[,3], y = (abs((predict(sp[[i]],user_data[,3],deriv=1)$y))), xout = B.r, method = "linear")$y) #derivatives of spline fits from sp list
              } else({if(input$smooth_gradient==TRUE){
                z[i] = list(approx(x = user_data[,3], y = (smooth.spline(x = user_data[,3], y = unlist(sp[[i]]), spar=sf)$y), xout = B.r, method = "linear")$y) #this needs to be the smooth.spline through the raw derivative
              }
              })
            }
            else({
              if(input$smooth_magnetization == TRUE){
                z[i] = list(approx(x = user_data[,1], y = (abs((predict(sp[[i]],user_data[,1],deriv=1)$y))), xout = B.r, method = "linear")$y)
              } else({
                if(input$smooth_gradient == TRUE){ z[i] = list(approx(x = user_data[,1], y = (smooth.spline(x = user_data[,1], y = unlist(sp[[i]]), spar=sf)$y), xout = B.r, method = "linear")$y) #this needs to be the smooth.spline through the raw derivative z[i] = list(approx(x = user_data[,1], y = smooth.spline(unlist(s[[i]][1]),(abs((predict(list(smooth.spline(x=unlist(s[[i]][1]),y=unlist(s[[i]][2]),spar=0))[[i]],user_data[,1],deriv=1)$y))))$y, xout = B.r, method = "linear")$y)
                }
              })
            })
          }
          ...
          if(n.comp == 1){
            ...
            user_data.a.m = auc(B.r,H_mean,from = min(B.r), to = max(B.r),type="spline")
            user_data.a.l = auc(B.r,H.l,from = min(B.r), to = max(B.r),type="spline")
            user_data.a.u = auc(B.r,H.u,from = min(B.r), to = max(B.r),type="spline")
            ## comp 1 ##
          ...
          if(n.comp == 1){
            ...
            #user_data.a.m.ec = auc(B.r.ec,H_mean,from = min(B.r), to = max(B.r.ec),type="spline")
            #user_data.a.l.ec = auc(B.r.ec,H.l,from = min(B.r), to = max(B.r.ec),type="spline")
            #user_data.a.u.ec = auc(B.r.ec,H.u,from = min(B.r), to = max(B.r.ec),type="spline")
            ## comp 1 ##
          ...
          if(n.comp == 3){
            ...
            mod.a.l.ec = auc(B.r.ec,G.model.op.l.ec,from = min(B.r.ec),to = max(B.r.ec),type = "spline")
            mod.a.u.ec = auc(B.r.ec,G.model.op.u.ec,from = min(B.r.ec),to = max(B.r.ec),type = "spline")
            # user_data.a.m.ec = auc(B.r.ec,H_mean,from = min(B.r), to = max(B.r),type="spline")
            #user_data.a.l.ec = auc(B.r.ec,H.l,from = min(B.r), to = max(B.r),type="spline")
            #user_data.a.u.ec = auc(B.r.ec,H.u,from = min(B.r), to = max(B.r),type="spline")
            ## comp 1 ##
            C1.op.m.a.ec = auc(B.r.ec,C1.op.m.ec,from = min(B.r.ec), to = max(B.r.ec),type="spline")
            C1.op.l.a.ec = auc(B.r.ec,C1.op.l.ec,from = min(B.r.ec), to = max(B.r.ec),type="spline")
            C1.op.u.a.ec = auc(B.r.ec,C1.op.u.ec,from = min(B.r.ec), to = max(B.r.ec),type="spline")
          ...
          output$final.results <- renderPrint({
            ...
            if(n.comp ==1){
              if(input$skew.option == FALSE){
                final.results[,1]<- c(p.final[1,1],p.final[1,2],
                                      p.final[2,1], p.final[2,2],
                                      p.final[3,1], p.final[3,2],
                                      p.final[4,1],p.final[4,2],
                                      TC[1,1],
                                      (abs(TC[1,1]-TC[1,2])+
                                         abs(TC[1,1]-TC[1,3])/2),
                                      EC[1,1],(abs(EC[1,1]-EC[1,2])+
                                                 abs(EC[1,1]-EC[1,3])/2))
              } else
                ({
                  final.results[, 1] <-
                    c(
                      ...
                      p.final[3, 2],
                      TC[1, 1],
                      (abs(TC[1, 1] - TC[1, 2]) + abs(TC[1, 1] - TC[1, 3]) / 2),
                      EC[1, 1],
                      (abs(EC[1, 1] - EC[1, 2]) + abs(EC[1, 1] - EC[1, 3]) / 2)
                    )
                })
            }
            if (n.comp == 2) {
              if (input$skew.option == FALSE) {
                ...
                final.results[, 2] <-
                  c(
                    ...
                    p.final[8, 2],
                    TC[2, 1],
                    (abs(TC[2, 1] - TC[2, 2]) + abs(TC[2, 1] - TC[2, 3]) / 2),
                    EC[2, 1],
                    (abs(EC[2, 1] - EC[2, 2]) + abs(EC[2, 1] - EC[2, 3]) / 2)
                  )
              } else
                ({
                  final.results[, 1] <-
  ...
  output$pdfviewer <- renderText({
    return(
      paste(
        '<iframe style="height:800px; width:100%" src="',
        "manual.pdf",
        '"></iframe>',
        sep = ""
      )
