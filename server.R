...
shinyServer(function(input, output) {
  ...
  dat <- reactive({
    if (input$example.data == TRUE) {
      dat = read.csv("PCB-01-TRB-050.txt",
                     sep = "",
                     header = F)
      colnames(dat) <- c("B", "M")
    } else
    ...
    dat[,4] <- smooth.spline(log(dat[,1],10),
                             dat[,2],
                             spar=sf)$y #V4
    dat[,5] <- abs(predict(smooth.spline(log(dat[,1],10),
                                         dat[,2],
                                         spar=sf),
                           deriv=1)$y) #V5
    dat[,6] <- smooth.spline(dat[,1],
                             dat[,2],
                             spar=sf)$y #V6
    dat[,7] <- abs(predict(smooth.spline(dat[,1],
                                         dat[,2],
                                         spar=sf),
                           deriv=1)$y) #V7
    dat[,8] <- abs(predict(smooth.spline(log(dat[,1],10),
                                         dat[,2],
                                         spar=0),
                           deriv=1)$y) #V8
    dat[,9] <- abs(predict(smooth.spline(dat[,1],
                                         dat[,2],
                                         spar=0),
                           deriv=1)$y) #V9
    dat[,10] <- smooth.spline(log(dat[,1],10),
                              dat[,8],
                              spar=sf)$y #V10
    dat[,11] <- smooth.spline(dat[,1],
                              dat[,9],
                              spar=sf)$y #V11
    dat
  })
...
# observeEvent(dat(),{vals$keeprows <- rep(TRUE, nrow(dat()))}) 
        ...
        if (input$plot.type == "magnetization data") {
          ...
          ({
            plot(
              ...
              type = "p",
              pch = 19,
              xlim = c(min(dat[, 3]), max(dat_keep[, 3])),
    ...
    if (n.comp == 4) {
      if (input$skew.option == FALSE) {
        ...
        C[, 4] = Pi[12] * (max(G.sm) * (dsnorm(B.d, Pi[10], Pi[11], S[4]) /
                                          max(dsnorm(B.d, Pi[10], Pi[11], S[4]))))
      } else
        ({
          C[, 1] = Pi[3] * (max(G.sm) * (dnorm(B.d, Pi[1], Pi[2]) / max(dnorm(B.d, Pi[1], Pi[2]))))
          C[, 2] = Pi[6] * (max(G.sm) * (dnorm(B.d, Pi[4], Pi[5]) / max(dnorm(B.d, Pi[4], Pi[5]))))
    ...
    if (n.comp == 5) {
      if (input$skew.option == FALSE) {
        ...
        C[, 5] = Pi[15] * (max(G.sm) * (dsnorm(B.d, Pi[13], Pi[14], S[5]) /
                                          max(dsnorm(B.d, Pi[13], Pi[14], S[5]))))
      } else
        ({
          C[, 1] = Pi[3] * (max(G.sm) * (dnorm(B.d, Pi[1], Pi[2]) / max(dnorm(B.d, Pi[1], Pi[2]))))
          C[, 2] = Pi[6] * (max(G.sm) * (dnorm(B.d, Pi[4], Pi[5]) / max(dnorm(B.d, Pi[4], Pi[5]))))
    ...
    if (n.comp == 6) {
      if (input$skew.option == FALSE) {
        ...
        C[, 6] = Pi[18] * (max(G.sm) * (dsnorm(B.d, Pi[16], Pi[17], S[6]) /
                                          max(dsnorm(B.d, Pi[16], Pi[17], S[6]))))
      } else
        ({
          C[, 1] = Pi[3] * (max(G.sm) * (dnorm(B.d, Pi[1], Pi[2]) / max(dnorm(B.d, Pi[1], Pi[2]))))
          C[, 2] = Pi[6] * (max(G.sm) * (dnorm(B.d, Pi[4], Pi[5]) / max(dnorm(B.d, Pi[4], Pi[5]))))
